...
const Mail = require("../notify/email");
const Telegram = require("../notify/telegram");
const SMS = require("../notify/sms");
...
module.exports = {
  boot: async function (projectDir) {
    ...
    try {
      config = JSON.parse(
        fs.readFileSync(`${parameters.projectDir}/conf.json`, "utf8")
      );
    } catch (e) {
    ...
    try {
      if (!fs.existsSync(`${parameters.projectDir}/bot.db`)) {
        execSync("sqlite3 bot.db < seed.sql");
      }
    } catch (e) {
      throw new Error(`Invalid bot.sql file. Please check: ${String(e)}`);
    }
  ...
  getStopLossCalculator: function () {
    ...
    return (stopLossCalculator = new StopLossCalculator(
      this.getTickers(),
      this.getLogger()
    ));
  },
  ...
  getRiskRewardRatioCalculator: function () {
    ...
    return (riskRewardRatioCalculator = new RiskRewardRatioCalculator(
      this.getLogger()
    ));
  },
  ...
  getCreateOrderListener: function () {
    ...
    return (createOrderListener = new CreateOrderListener(
  ...
  getNotifier: function () {
    ...
    const mailEnable = _.get(config, "notify.mail.enable");
    if (mailServer && mailServer.length > 0 && mailEnable) {
      notifiers.push(
        new Mail(this.createMailer(), this.getSystemUtil(), this.getLogger())
      );
    }
    ...
    if (
      ...
      notifiers.push(
        new Telegram(this.createTelegram(), telegram, this.getLogger())
      );
    }

    const sms = _.get(config, "notify.sms");
    if (sms.from && sms.to && sms.enable) {
      notifiers.push(
        new SMS(this.createSMS(), this.getSystemUtil(), this.getLogger())
      );
    }
  ...
  getTickerRepository: function () {
    ...
    return (tickerRepository = new TickerRepository(
      this.getDatabase(),
      this.getLogger()
    ));
  },
  ...
  createMailer: function () {
    ...
    return mail.createTransport(
      `smtps://${config.notify.mail.username}:${config.notify.mail.password}@${
        config.notify.mail.server
      }:${config.notify.mail.password || 465}`,
      {
  ...
  createSMS: function () {
    const config = this.getConfig();
    const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN } = config.notify.sms;
    const client = require("twilio")(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN);
    if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
      console.log("Twilio: No api token given");
      return;
    }
