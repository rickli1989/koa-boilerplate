...
module.exports = class Binance {
	...
	start(config, symbols) {
		...
		// 		setTimeout(async () => {
		// 			const initSymbols = (await me.getPositions()).map((p) =>
		// 				p.getSymbol()
		// 			);
		// 			me.logger.info(
		// 				`Binance: init trades for positions: ${JSON.stringify(initSymbols)}`
		// 			);
		...
		symbols.forEach((symbol) => {
			...
			symbol.periods.forEach((interval) => {
				...
				this.queue.add(() => {
					client
						.candles({ symbol: symbol.symbol, limit: 500, interval: interval })
						.then(async (candles) => {
	...
	async order(order) {
		...
		try {
			...
			if (
				...
				return ExchangeOrder.createRejectedFromOrder(
					order,
					`${e.code} - ${e.message}`
				);
			}
	...
	async cancelOrder(id) {
		...
		try {
			...
			if (message.includes("unknown order sent")) {
				this.logger.info(
					`Binance: cancel order not found remove it: ${JSON.stringify([
						e,
						id,
					])}`
				);
			...
			this.logger.error(
				`Binance: cancel order error: ${JSON.stringify([e, id])}`
			);
	...
	static createOrders(...orders) {
		return orders.map((order) => {
			...
			if (amount < 0) {
				throw new Error(
					`Invalid order amount: ${JSON.stringify(amount, order)}`
				);
			}
	...
	calculatePrice(price, symbol) {
		if (
			!(symbol in this.exchangePairs) ||
			!this.exchangePairs[symbol].tick_size
		) {
			return undefined;
		}

		return OrderUtil.calculateNearestSize(
			price,
			this.exchangePairs[symbol].tick_size
		);
	}
	...
	async getPositions() {
		...
		this.symbols
			...
			.forEach((s) => {
				if (s.trade.capital > 0) {
					...
					s.trade.currency_capital > 0 &&
					this.tickers[s.symbol] &&
					this.tickers[s.symbol].bid
				) {
					capitals[s.symbol] =
						s.trade.currency_capital / this.tickers[s.symbol].bid;
				} else {
					capitals[s.symbol] = 0;
				}
			});
		...
		for (const balance of this.balances) {
			...
			for (const pair in capitals) {
				...
				if (
					!pair.startsWith(asset) ||
					pair.startsWith(`${asset}UP`) ||
					pair.startsWith(`${asset}DOWN`)
				) {
					continue;
				}
				...
				if (trades && trades.length > 0) {
					const positionEntry = TradesUtil.findPositionEntryFromTrades(
						trades,
	...
	async updateOrder(id, order) {
		...
		return this.order(
			Order.createUpdateOrderOnCurrent(currentOrder, order.price, order.amount)
		);
	}
	...
	async onWebSocketEvent(event) {
		if (
			event.eventType &&
			event.eventType === "executionReport" &&
			("orderStatus" in event || "orderId" in event)
		) {
			this.logger.debug(
				`Binance: Got executionReport order event: ${JSON.stringify(event)}`
			);
			...
			if (isRemoveEvent) {
				this.logger.info(
					`Binance: Removing non open order: ${orderStatus} - ${JSON.stringify(
						event
					)}`
				);
				this.orderbag.delete(event.orderId);
				this.throttler.addTask(
					"binance_sync_balances",
					this.syncBalances.bind(this),
					5000
				);
			}
		...
		if (
			event.eventType &&
			["outboundAccountPosition", "account", "balanceUpdate"].includes(
				event.eventType
			)
		) {
			this.throttler.addTask(
	...
	async syncTradesForEntries(symbols = []) {
		...
		if (symbols.length === 0) {
			...
			const positionSymbols = _.shuffle(
				(await this.getPositions()).map((p) => p.getSymbol())
			);
			const unknown = _.shuffle(allSymbols).filter(
		...
		this.logger.debug(
			`Binance: Sync trades for entries: ${symbols.length} - ${JSON.stringify(
				symbols
			)}`
		);
		...
		const promises = symbols.map((symbol) => {
			return async () => {
				...
				try {
					...
					this.logger.info(
						`Binance: Sync trades error for entries: ${symbol} - ${String(e)}`
					);
					return undefined;
				}
